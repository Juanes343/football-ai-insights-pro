...
import { authenticate, optionalAuthenticate, requirePremium } from '../middleware/auth';
import { predictionLimiter } from '../middleware/rateLimit';

const router = Router();

router.get('/match/:matchId', optionalAuthenticate, predictionLimiter, ctrl.getPredictionForMatch);
